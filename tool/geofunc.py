...
class GeoFunc(object):
    ...
    def intersection(line1,line2):
        ...
        # print(f'inter : {inter}')
        # exit()
        if inter.is_empty==False: # 交点が存在する場合
            mapping_inter=mapping(inter)
            if mapping_inter["type"]=="LineString": # 線分重複の場合：最初の点
                inter_coor=mapping_inter["coordinates"][0] 
            else:
                inter_coor=mapping_inter["coordinates"] # 単一交点の場合：その座標
            return inter_coor
        ...
        for pt1 in line1:
            for pt2 in line2:
                if GeoFunc.almostEqual(pt1,pt2)==True:  # 近似的に等しいか
        ...
        # almostContainが存在するかを計算
        for pt in line1:
            if GeoFunc.almostContain(line2,pt)==True:
                return pt # line1の端点がline2上にある
        for pt in line2:
